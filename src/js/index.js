...
class Main {
  ...
  init() {
    ...
    this.loader
      ...
      .add({ name: 'GROBOLD', url: 'assets/fonts/GROBOLD.ttf' })
  ...
  setButton() {
    ...
    btn_retry.position.set(this.app.screen.width * 0.1, (this.app.screen.height * 9.3 / 10) - btn_retry.height * 1.5);
  ...
  removePlus() {
    ...
    if (plusAds) this.buttonContainer.removeChild(plusAds)
  }
  drawTextBack() {
  ...
  resize() {
    ...
    this.app.renderer.resize(window.innerWidth, window.innerHeight);
    this.game.pixiRemoveAllChildren(this.game.containerMain)
    this.setBackGround()
    while (this.buttonContainer.children[0]) {
      this.buttonContainer.removeChild(this.buttonContainer.children[0]);
    }
